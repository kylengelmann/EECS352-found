...
def load_wav(filename):
    try:
        (wavedata, samplerate)=librosa.core.load(filename, sr=44100)
...
def paulstretch(samplerate,smp,stretch,windowsize_seconds):
    ...
    out = zeros(len(smp)*stretch + windowsize)
    ...
    while True:
        ...
        # out = concatenate((out, output))
        out[outstart:outstart+len(output)] = output
        outstart += len(output)
